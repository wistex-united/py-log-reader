...
class TypeInfoChunk(Chunk):
    ...
    def registerClasses(self):
        if hasattr(self, "_dataClasses") and self._dataClasses is not None:
            for (
                className,
                classType,
            ) in (
                self._dataClasses.items()
            ):  # This is a hack to make pickle find the class type it need in Type
                setattr(self, className, classType)
